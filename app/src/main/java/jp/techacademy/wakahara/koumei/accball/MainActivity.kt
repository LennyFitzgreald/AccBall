package jp.techacademy.wakahara.koumei.accball
...
class MainActivity : AppCompatActivity(), SensorEventListener, SurfaceHolder.Callback {
    ...
    private var radius = 50.0f         // ボールの半径を表す定数
    private var coef = 1000.0f          // ボールの移動量を調整するための計数
    ...
    override fun onSensorChanged(event: SensorEvent?) {
        ...
        if (event.sensor.type == Sensor.TYPE_ACCELEROMETER) {
            ...
            if (ballX - radius < 0 && vx < 0) {
                vx = -vx /1.5f
                ballX = radius
            } else if (ballX + radius > surfaceHeight && vx > 0) {
                vx = -vx / 1.5f
